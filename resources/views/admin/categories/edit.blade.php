...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="bg-white dark:bg-[#161615] shadow-md rounded-lg p-6">
            <h1 class="text-2xl font-bold text-[#1b1b18] dark:text-[#EDEDEC] mb-6">Edit Category</h1>
            ...
            <form action="{{ route('admin.categories.update', $category) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea 
                        ...
                        name="description" 
                        rows="3"
                        class="w-full p-2 border border-gray-300 rounded-md bg-[#FDFDFC] dark:bg-[#1E1E1C] dark:border-[#3E3E3A] dark:text-[#EDEDEC] focus:ring-[#f53003] focus:border-[#f53003]">{{ old('description', $category->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="flex items-center space-x-4">
                        <label class="inline-flex items-center">
                            <input type="radio" name="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }} class="text-[#f53003] focus:ring-[#f53003]">
                            <span class="ml-2 text-[#1b1b18] dark:text-[#EDEDEC]">Active</span>
                        </label>
                        <label class="inline-flex items-center">
                            <input type="radio" name="is_active" value="0" {{ old('is_active', $category->is_active) ? '' : 'checked' }} class="text-[#f53003] focus:ring-[#f53003]">
