    ...
    <div class="mb-6 bg-[#f53003] text-white rounded-lg p-4 shadow">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-sm opacity-80">Meja: {{ $order->table_number }} | Status: {{ ucfirst(str_replace('_', ' ', $order->status)) }}</p>
            </div>
        ...
        <div class="bg-white dark:bg-[#161615] rounded-lg shadow p-6">
            ...
            <div class="mb-8">
                ...
                <div class="w-full bg-gray-200 dark:bg-[#1E1E1C] rounded-full h-2.5">
                    <div class="h-2.5 rounded-full bg-[#f53003]" id="progressBar" style="width: {{ $order->status === 'pending_payment' ? '25' : ($order->status === 'processing' ? '50' : ($order->status === 'preparing' ? '75' : ($order->status === 'ready' ? '90' : '100'))) }}%"></div>
                </div>
            </div>
            ...
            <div class="space-y-4 mb-6">
                <div class="flex items-start">
                    <div class="mr-4">
                        <div class="w-8 h-8 rounded-full bg-[#f53003] text-white flex items-center justify-center">
                            @if(in_array($order->status, ['pending_payment', 'processing', 'preparing', 'ready', 'completed']))
                ...
                <div class="flex items-start">
                    <div class="mr-4">
                        <div class="w-8 h-8 rounded-full @if(in_array($order->status, ['processing', 'preparing', 'ready', 'completed'])) bg-[#f53003] text-white @else bg-gray-300 text-[#1b1b18] @endif flex items-center justify-center">
                ...
                <div class="flex items-start">
                    <div class="mr-4">
                        <div class="w-8 h-8 rounded-full @if(in_array($order->status, ['preparing', 'ready', 'completed'])) bg-[#f53003] text-white @else bg-gray-300 text-[#1b1b18] @endif flex items-center justify-center">
                ...
                <div class="flex items-start">
                    <div class="mr-4">
                        <div class="w-8 h-8 rounded-full @if(in_array($order->status, ['ready', 'completed'])) bg-[#f53003] text-white @else bg-gray-300 text-[#1b1b18] @endif flex items-center justify-center">
            ...
            <div class="bg-[#FDFDFC] dark:bg-[#1E1E1C] rounded-lg p-4 mb-6">
                ...
                <div class="border-t border-gray-200 dark:border-[#3E3E3A] pt-3 mt-3">
                    <div class="flex justify-between mb-1">
                        ...
                        <span class="text-[#1b1b18] dark:text-[#EDEDEC]">Rp {{ number_format($order->subtotal, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between mb-1">
                        <span class="text-[#1b1b18] dark:text-[#EDEDEC]">Pajak (11%):</span>
                        <span class="text-[#1b1b18] dark:text-[#EDEDEC]">Rp {{ number_format($order->tax, 0, ',', '.') }}</span>
                    </div>
